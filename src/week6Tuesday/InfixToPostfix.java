...
public class InfixToPostfix {
	...
	public static void main(String[] args) 
	{
		...
		iTf.processToStack(input);
	}

	public void processToStack(char[] input) {
		for (char c : input) {
	...
	public boolean isOperatorP1(char c) {
		for (int i = 0; i < oppref1.length; i++) 
			if(c==oppref1[i])
				return true;
		return false;
	}
	public boolean isOperatorP2(char c) {
		for (int i = 0; i < oppref2.length; i++) 
			if(c==oppref2[i])
				return true;
		return false;
	}
	public boolean isOperatorP3(char c) {
		for (int i = 0; i < oppref3.length; i++) 
			if(c==oppref3[i])
				return true;
		return false;
	}
	public String processToPostfix(char[] input) {
		for (char c : input) {
			if(isOperatorP1(c) || isOperatorP2(c) || isOperatorP3(c)){
				if(tempCnt==0){
					temp[tempCnt]=c;
